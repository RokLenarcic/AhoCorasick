...
// A map that matches only when one of the strings in the dictionary
// matches the whole input string.
// Should take up less space than a hashmap when the words in the set
// have a lot of same prefixes.
public class FullStringMatchMap<T> implements StringMap<T> {
    ...
    public void match(final String haystack, final MapMatchListener<T> listener) {
        ...
        if (caseSensitive) {
            ...
            if (currentNode.isEmpty()) {
                listener.match(0, len, currentNode.value);
            }
        } else {
            while (idx < len) {
                // Try to transition from the current node using the character
